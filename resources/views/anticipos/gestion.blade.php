      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{url('anticipos/')}}/rechazar" method="POST" enctype="multipart/form-data">
            ...
            <label for="exampleFormControlTextarea1">Por favor ingresa el motivo del rechazo del anticipo:</label>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                  Anticipos por gestionar
                </div>
                <div>
                <!--
                  EJEMPLOS Llamados a Funciones desde el Config/app.php
                {{ $_SERVER['HTTP_HOST'] }} - <br />
                {{ config('app.getUrlBase') }} - 
                {{ config('app.logg')("IP:".$_SERVER['HTTP_HOST']) }}
                -->
                </div>
                    ...
                    <table class="table-responsive-md table-bordered table-striped table-sm" cellspacing="0" width="100%">
                        ...
                        @foreach($anticipos as $anticipo)
                              ...
                              <td>
                                ...
                                onclick='CargarModalFiles("{{$anticipo->id}}");'>
                              </td>
                              <td>{{$anticipo->estado}}</td>
                              <td>
                                <a href="{{url('anticipos/')}}/{{$anticipo->id}}/{{$anticipo->id_user}}/aceptar" 
                              ...
                              <td>
                                ...
                                onclick='CargarMotivoRechazo("{{$anticipo->id}}","{{$id_usuario}}");'>
                              </td>
                            </tr>
                        @endforeach
                        </tbody>
...
function CargarModalFiles(id){
  ...
  $.ajax({
      ...
      url: urlBase_ + '/api/adjuntosfilesanticipos',
      type:'POST',
      dataType :'JSON',
