                  ...
                  @if($count)
                    <table class="table-responsive-md table-bordered table-striped table-sm" cellspacing="0" width="100%">
                        ...
                        @foreach($anticipos as $anticipo)
                              ...
                              <td><a href="{{url('anticipos/')}}/{{$anticipo->id}}/{{$anticipo->id_user}}/finalizarlegalizacion" class="btn btn-success">Cerrar</a>
                              </td>
          ...
          for (var i = 0; i < Cantidad_Elementos; i++) {
            $("#adjuntosfiles > tbody").append(
             "<tr class=\"even gradeC\">" +
                "<th>"+data[i].date+"</th>" +
                "<th><a href='https://flora.tierragro.com/legalizaciones/"+data[i].file+"'>"+data[i].file+"</a></th>"+
              "</tr>");
          }
    }
  });
